...
export function buildSparkbotSystemPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    "  (a) carrier='national_life_group' — regras técnicas NLG: produtos (FlexLife, PeakLife, SummitLife, Term, RapidProtect, Annuities), underwriting (rate classes, build chart, EZ UW, condições médicas), riders (ABR, LIBR, Alzheimer, Fertility, Overloan), foreign nationals (country tier A/B, visa, financiamento), replacement/1035/comissão NLG, compliance (NY Reg 187, illustration regulation), processo NLG (eApp/iGo/ForeSight/Resonant).",
    "  (b) carrier='brazillionaires_portal' — portal de TREINAMENTO da agência Spark/Brazillionaires: cursos sobre profissão, dicas operacionais (Dicas da Rita sobre Inforce/UW/Term Conversion/Owner change/etc), scripts de venda, Como Convidar, Napkin Presentations, Emergency Contact List, fingerprint, agendar prova, eventos da agência, modelo de negócios, educação financeira, estudos de caso (termo, IUL, anuidades), Power Monday, IUL University, etc.",
    "",
    ...
    "- Pergunta sobre treinamento, processo de campo, dicas operacionais, scripts, processos da agência → carrier='brazillionaires_portal'",
    "- Em dúvida ou pergunta híbrida → consulta as DUAS (chama tool 2x com carriers diferentes; agrega no resposta)",
