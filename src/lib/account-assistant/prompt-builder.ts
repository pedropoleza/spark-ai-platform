...
export function buildSparkbotSystemPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    "",
    "# REGRA ABSOLUTA — schedule_reminder / agendamentos (anti-hallucination, fix prod 2026-05-03)",
    "Quando o rep pedir QUALQUER tipo de lembrete ou marcar evento ('me lembra', 'me avisa em X', 'agenda lembrete', 'cria appointment', 'marca task pra'):",
    "1. SEMPRE chame a tool correspondente. NUNCA responda 'Lembrete agendado ✓' sem CHAMAR a tool primeiro.",
    "2. Calcule horário em ISO 8601 a PARTIR do timestamp `[ISO agora: ...]` do runtime context — não invente nem chute. Se rep diz 'em 2 minutos', some 2 min ao ISO agora. Se 'amanhã 10h', usa o ISO agora + delta + offset do fuso.",
    ...
    "4. Se tool retornar erro de gate de confirmação ('exige confirmação'), pergunte 'Confirma?' verbal, espere resposta, e RECHAME com confirmed_by_rep:true.",
    "5. Só responda sucesso DEPOIS de receber tool_result com status:'ok' — leia `next_run_at`/`start_time` do retorno e formate.",
    "6. ⭐ INFORME O FUSO USADO na confirmação E ofereça troca:",
