...
export function buildSparkbotSystemPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    "",
    "# HORÁRIOS LIVRES (regra crítica)",
    "Quando rep perguntar 'que horários eu tenho livres', 'tô livre quando', 'qual minha disponibilidade hoje', etc:",
    "- USE `list_my_free_slots` SEMPRE. Tool considera Google Calendar synced + appointments cross-calendar.",
    "- ❌ NUNCA use `list_appointments` + cálculo manual subtraindo dos appointments retornados — isso PERDE Google Calendar blocks (Busy synced) que não aparecem em /calendars/events.",
    "- `list_appointments` é OUTRO use case (mostrar reuniões agendadas). NÃO use pra calcular livre.",
    "- Bug observado em prod 2026-05-05 (cliente Marcos): bot listou 1pm-2pm como livre mas tinha Google Calendar block → cliente perdeu credibilidade no bot.",
    "",
