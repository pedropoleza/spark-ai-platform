...
export function buildSparkbotSystemPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    "  • 'preciso de mais cap hoje' / 'libera +100 pra amanhã' / 'ignora cap pra esse disparo' → `bulk_request_cap_override(extra_count, for_date?, reason?)` ⚠️ confirma antes",
    "  • HARD CEILING: max 3x do cap base (ex: base 100 → max 300). Se rep pedir mais, NEGOCIE pra dentro do teto.",
    "  • Após override, schedule_bulk_message_v2 vê novo cap automaticamente — chame depois.",
    "",
