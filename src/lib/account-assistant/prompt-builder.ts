...
export function buildSparkbotSystemPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    "  NOT:    { \"not\": filtro1 }",
    "",
    "FIELDS comuns: firstName, lastName, email, phone, tags, dateOfBirth, dateAdded, lastActivity, state, city, source, assignedTo, dnd, opportunity.stageName (alias auto), opportunity.stageId, opportunity.status, opportunity.monetaryValue, opportunity.assignedTo, customField.{slug ou UUID}",
    ...
    "5. Custom fields: rep pode falar nome ('product categories') ou slug ('contact.client_product_categories') — engine resolve. Use describe_filter_capabilities pra ver opções disponíveis na location.",
    ...
    "",
    "# BULK MESSAGES V2 — disclaimers tier (H28)",
    "Pra disparo em massa preferencial: `preview_bulk_message_v2` + `schedule_bulk_message_v2` (novos). Aceitam multi-segment (1 job com N filters × N templates).",
    "",
    "FLUXO OBRIGATÓRIO:",
    "1. Bot pergunta ao rep: 'É lista QUENTE (pessoas que já interagiram) ou FRIA (sem interação prévia)?'",
    "2. Chama `preview_bulk_message_v2(segments=[...], list_temperature='warm'|'cold')`",
    "3. Preview retorna `disclaimers[]` — bot EXIBE CADA UM separadamente pro rep (use SPLITTER `---`), aguarda confirmação textual",
    "4. Após TODOS confirmados, chama `schedule_bulk_message_v2` com flags de aceite (confirmed_warm_list, confirmed_risk_cold, etc)",
    "",
