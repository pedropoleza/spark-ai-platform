...
const queryCarrierKnowledge: ToolEntry = {
  def: {
    ...
    description:
      // B2 (Onda B 2026-07-21): dieta — mantém o roteamento essencial (2 KBs + 2 exemplos
      // âncora + regra do híbrido); o contexto da operação mora no overview de carrier do
      // system (tier-1). Anti-alucinação preservada ("NUNCA invente sem chamar").
      "Consulta uma das DUAS knowledge bases (param `kb`): 'national_life_group' = regras TÉCNICAS da carrier NLG (produtos FlexLife/PeakLife/Term/Annuities, UW/rate classes, riders, compliance, FN/visas — ex: 'qual cap do FlexLife em NY?'); 'agency_brazillionaires' = treinamento/operação da sub-agência (Napkin, Como Convidar, prova de licença, scripts, Dicas da Rita — ex: 'como funciona a Napkin Presentation?'). Em dúvida ou pergunta híbrida, chame DUAS VEZES (uma com cada kb). NUNCA invente resposta de carrier sem chamar esta tool.",
    risk: "safe",
    parameters: {
      type: "object",
      properties: {
