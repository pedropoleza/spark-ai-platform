...
### ✅ Ultra-review + prompt plugado (2026-06-21, commit 2064680)
Review multi-agente (37 agentes, 7 dimensões, cada achado verificado: **24/29 confirmados**, 5 refutados). **Gap nº1 = prompt não plugado** → RESOLVIDO: seção `# MONTAR FLUXO DE FOLLOW-UP GRANDE` em `prompt-builder.ts`, GATED pela flag (OFF = prompt idêntico). **Fix-now aplicados:** IDOR (rep_id nos resolvers), cap apply (200/2000 + validateGhlId), idempotência (anti-duplicata draft+contato), `intra_day_delay_s` (era ignorado), signals de falha (materialize/PDF), legenda vazia ≠ URL. **Defer (follow-up):** `assignedTo` no followup-runner compartilhado (blast radius em sales/recrut), allowlist `media_url`, recovery de 'materializing' preso, location-drift, interpolação `{first_name}`, recorrência cíclica, `send_condition` per-passo, UI no /hub, upload de mídia recebida. **5 refutados:** double-commit guard (CAS OK), race cron-vs-msgs (sequence antes das msgs mas runner só pega `scheduled`), orphan-sends (insert de array é atômico → 0 ou tudo). 50 unit + 18 smoke + tsc + build. Doc: H41 (CLAUDE.md + DECISIONS.md).

### 👤 Ainda pendente do Pedro (bloqueia o GO-LIVE)
1. **LLM dirigindo as tools** numa conversa real (mecânica provada ponta-a-ponta + prompt já plugado; falta ver o SparkBot deployado ESCOLHENDO as tools). Exige a flag ON.
