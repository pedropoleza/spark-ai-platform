...
| 2 | **Opt-out/STOP ignorado em envio agendado/recorrente direto ao contato** (TCPA/LGPD) | ✅ **FECHADO** no `reminder-runner.fireOutboundToContact` (gate `filterOutOptOutContacts` + cancela a task). ⚠️ Follow-up: tools `send/schedule_message_to_contact` (rep-iniciado) ainda sem gate |
...
| 5 | **Cron de refresh sem deadman + env sem `GHL_CLIENT_ID/SECRET`** | 🟡 **Parcial**: `reportError` no cron (side-task). **Falta:** validar `GHL_CLIENT_ID/SECRET` no `env.ts` no boot + deadman no `/api/health` |
...
- **`move_opportunity` rejeita stage_id UUID com hífen** → mover etapa falha sempre. → 🔴 aberto (fix trivial).
