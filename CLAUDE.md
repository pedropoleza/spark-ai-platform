...
- **Status**: Fase 0 PRONTA + Fase 1 base (motor seam + 4 módulos + registry + harness) PRONTAS e deployadas, **zero mudança de comportamento** (flags OFF). Próximo: Fase 2 (lead-facing como templates no motor + multicanal) precisa de decisões de design com o Pedro. **NÃO** ligar `AGENT_ENTITLEMENTS_ENFORCED` (até UI de liberação, Fase 3) nem `AGENT_MOTOR_UNIFIED` em prod sem validar paridade ao vivo primeiro.
