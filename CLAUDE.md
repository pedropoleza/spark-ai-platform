...
- **Pra ligar em prod (👤)**: `TASK_ORCHESTRATOR_ENABLED=1` na Vercel (JÁ LIGADA — a Jussara usa em prod) + validar **1 conversa real com o LLM dirigindo as tools**. Teste: `scripts/test-task-orchestrator.ts` (69/69) + `scripts/smoke-task-orchestrator.ts` (18/18, via `executeTool`).
